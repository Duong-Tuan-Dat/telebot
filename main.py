...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if step == "waiting_for_photo":
            if ("duong tuan dat" in cleaned_text or "dương tuấn đạt" in cleaned_text) and "50" in cleaned_text:
...
Nếu có thắc mắc, xin liên hệ qua Telegram: @quanlyngocnhu hoặc hotline: 0388 188 655 (5.000vnđ/phút) để được hỗ trợ.""")
        ...
        # Bước 2
            ...
            if ("duong tuan dat" in cleaned_text or "dương tuấn đạt" in cleaned_text) and "50" in cleaned_text and "598" in cleaned_text:
                last_msg_id = user_state.get(chat_id, {}).get("last_msg_id")
                if last_msg_id:
                    await context.bot.edit_message_text(chat_id=chat_id, message_id=last_msg_id, text="""Hướng dẫn nhận thưởng và tham gia nhóm làm việc
...
Nếu có thắc mắc, xin liên hệ qua Telegram: @quanlyngocnhu hoặc hotline: 0388 188 655 để được hỗ trợ.""")
        ...
        # Bước 3
            ...
            if ("duong tuan dat" in cleaned_text or "dương tuấn đạt" in cleaned_text) and "50" in cleaned_text and "589" in cleaned_text:
...
Nếu có thắc mắc, xin liên hệ qua Telegram: @quanlyngocnhu hoặc hotline: 0388 188 655 để được hỗ trợ.""")
        ...
        # Bước 4
            ...
            if ("duong tuan dat" in cleaned_text or "dương tuấn đạt" in cleaned_text) and "50" in cleaned_text:
                expected_case = "Id" if user_state[chat_id].get("last_id_case") == "id" else "id"
                if expected_case in original_text:
                    ...
                    user_state[chat_id]["step"] = "blocked"
                else:
                    await update.message.reply_text(
